...
export const command: Command = {
  runner: async function (appContext) {
    const logger = new Logger('fill-amount-scale');
    ...
    const rialUnitIds = new Set(
      units
        .filter((unit) => unit.symbol === RIAL_UNIT_SYMBOL)
        .map((unit) => unit.id),
    );

    const accountRows = await accountRepository.findAll({});
    const rialAccountIds = new Set(
      accountRows
        .filter((account) => rialUnitIds.has(account.unitId))
    ...
    const accountAffectedRows = await scaleRows(
      ...
      (row) =>
        rialAccountIds.has(row.id) ? { ballance: row.ballance * SCALE } : null,
    );
    ...
    const paymentAffectedRows = await scaleRows(
      ...
      (row) =>
        rialAccountIds.has(row.accountId)
    ...
    const accountDebtAffectedRows = await scaleRows(
      ...
      (row) => {
        ...
        return accountId !== undefined && rialAccountIds.has(accountId)
    ...
    const exchangeAffectedRows = await scaleRows(
      ...
      (row) => {
        ...
        const scaleFrom =
          fromAccountId !== undefined && rialAccountIds.has(fromAccountId);
        const scaleTo =
          toAccountId !== undefined && rialAccountIds.has(toAccountId);
    ...
    logger.log({
      key: 'FILL_AMOUNT_SCALE',
      data: {
        rialUnitCount: rialUnitIds.size,
        rialAccountCount: rialAccountIds.size,
        accountAffectedRows,
        paymentAffectedRows,
        incomeAffectedRows,
  ...
  cmd: 'fill-amount-scale',
  describe:
    'Multiplies every stored amount belonging to a Rial-unit account (payments, incomes, account balances, account debts, exchanges, uncomplete payments) by 10000, undoing the old trimmed-unit convention — non-Rial units are left untouched',
};
